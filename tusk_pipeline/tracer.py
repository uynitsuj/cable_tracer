...
from refine_push_location import find_crossings
...
class Tracer:
    def __init__(self) -> None:
        ...
        if use_cuda:
            ...
            self.trace_model.load_state_dict(torch.load('/home/mallika/triton4-lip/cable-untangling/untangling/tracer_knot_detect/models/tracer/tracer_model.pth')) # for athena5
        else:
    ...
    def _trace(self, image, start_points, exact_path_len, endpoints=None, viz=False, model=None, sample=False):    
        ...
        for iter in range(exact_path_len):
            ...
            n_model_output = model_output_flat / np.linalg.norm(model_output_flat, ord=1)
            k = 5
            ...
            # Calculate the mean
            m, n = p.shape
            mean = np.array([np.sum(np.array([i, j]) * p[i, j]) for i in range(m) for j in range(n)]).sum(axis=0) / (m * n)

            # Calculate the covariance matrix
            cov = np.zeros((2, 2))
            for i in range(m):
                for j in range(n):
                    cov += np.outer(np.array([i, j]) - mean, np.array([i, j]) - mean) * p[i, j]
            ...
            if sample:
                ...
                sample_output = np.random.choice(top_k_points, p=np.exp(top_k_probabilities) / np.sum(np.exp(top_k_probabilities)))
                # sample_output = np.random.choice(top_k_points, p=top_k_probabilities / np.sum(top_k_probabilities))
                yx = np.unravel_index(sample_output, model_output.shape)
            else:
                ...
                for endpoint in endpoints:
                    ...
                    if (abs(global_yx[0] - endpoint[0])) < self.ep_buffer and (abs(global_yx[1] - endpoint[1])) < self.ep_buffer:
                        if pix_dist > 0.8: # ~in meters (?)
                            max_sums = find_crossings(image, path)
                            return path, TraceEnd.ENDPOINT, heatmaps, crops, covariances, max_sums
            ...
            if len(path) > 10:
                ...
                for i in range(0, len(path)-20):
                    ...
                    if diff < 10 or diffrev < 30:
                        max_sums = find_crossings(image, path)
                        return path, TraceEnd.RETRACE, heatmaps, crops, covariances, max_sums
    ...
    def trace(self, img, prev_pixels, endpoints=None, path_len=20, viz=False, idx=0, folder=None, mask=False, sample=False):
        ...
        spline, trace_end, heatmaps, crops, covariances, max_sums = self._trace(img, starting_points, exact_path_len=path_len, endpoints=endpoints, model=self.trace_model, viz=False, sample=sample)
        ...
        if viz:
            ...
            if folder is not None:
                if mask:
                    plt.imsave(f'./{folder}/trace_{idx}_mask.png', trace_viz)
                else:
                    plt.imsave(f'./{folder}/trace_{idx}.png', trace_viz)
            else:
                if mask:
                    plt.imsave(f'./trace_{idx}_mask.png', trace_viz)
                else:
