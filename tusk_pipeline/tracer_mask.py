...
class Tracer:
    def __init__(self) -> None:
        ...
        if use_cuda:
            ...
            self.trace_model.load_state_dict(torch.load('../models/tracer/tracer_model.pth')) 
        else:
            ...
            self.trace_model.load_state_dict(torch.load('../models/tracer/tracer_model.pth', map_location=torch.device('cpu'))) # Uncomment for bajcsy
        augs = []
    ...
    def trace(self, img, prev_pixels, endpoints=None, path_len=20, viz=False, idx=0, folder=None, mask=False):
...
if __name__ == '__main__': 
    ...
    eval_folder = 'multicable_data/tier3'
    for i, data in enumerate(np.sort(os.listdir(eval_folder))):
        ...
        img = test_data['image']
        plt.imsave(f'./binary_mask_experiments_multicable_all/tier3/trace_{i}_img.png', img)
        thresh_img = np.where(img[:,:,:3] > 100, 255, 0).astype('uint8')
        # plt.imshow(thresh_img)
        # plt.plot(test_data['start_point'][0][0], test_data['start_point'][0][1], 'ro')
        # plt.show()
        start_pixels = np.array(test_data['start_point'][0], dtype=np.uint32)[::-1]
        print(start_pixels)
        ...
        spline = tracer.trace(img, start_pixels, path_len=200, viz=True, idx=i, mask=False, folder="binary_mask_experiments_multicable_all/tier3")
        spline = tracer.trace(thresh_img, start_pixels, path_len=200, viz=True, idx=i, mask=True, folder="binary_mask_experiments_multicable_all/tier3")
