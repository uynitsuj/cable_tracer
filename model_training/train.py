...
if not is_point_pred(config.expt_type):
    if use_cuda:
        ...
        keypoints = ClassificationModel(num_classes=config.classes, img_height=config.img_height, img_width=config.img_width, resnet_type=config.resnet_type)
else:
    if use_cuda:
        ...
        keypoints = KeypointsGauss(num_keypoints=1, img_height=config.img_height, img_width=config.img_width, resnet_type=config.resnet_type, pretrained=config.pretrained)
